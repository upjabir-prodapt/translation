...
def generate_report():
    ...
    measures_path = Path("/home/jupyter/Translation/sonar_measures.json")
    report_path = Path("/home/jupyter/Translation/sonarqube_detailed_report.md")

    if not issues_path.exists() or not measures_path.exists():
    ...
    files.sort(
        key=lambda x: int(
            get_metric(x, "ncloc") if get_metric(x, "ncloc") != "N/A" else 0
        ),
        reverse=True,
    )
