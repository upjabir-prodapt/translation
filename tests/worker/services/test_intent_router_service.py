...
class TestIntentRouterService:
    def test_build_intent(self, service):
        ...
        assert (
            service.build_intent("COMMERCIAL", "es", "ja") == "Intent-Commercial-ES-JA"
        )

    @patch("src.worker.services.intent_router_service.get_storage_client")
    @patch("src.worker.services.intent_router_service.get_cache_file_path")
    ...
    def test_get_model_chain(self, service):
        with patch(
            "src.worker.services.intent_router_service.select_model_list"
        ) as mock_select:
            mock_select.return_value = ["m1"]
            assert service.get_model_chain(
                domain="d", source_lang="en", target_lang="fr"
            ) == ["m1"]
    ...
    def test_local_cache_fresh_exists_and_expired(self, service):
        ...
        mock_file.stat.return_value.st_mtime = (
            datetime.now(UTC) - service._cache_ttl * 2
        ).timestamp()
        assert service._local_cache_fresh(mock_file) is False

    @patch(
        "src.worker.services.intent_router_service.IntentRouterService._load_from_local_fallback"
    )
    @patch(
        "src.worker.services.intent_router_service.IntentRouterService._local_cache_fresh",
        return_value=True,
    )
    def test_sync_model_selection_cache_no_force_fresh(
        self, mock_fresh, mock_local, service
    ):
    ...
    @patch(
        "src.worker.services.intent_router_service.IntentRouterService.sync_model_selection_cache"
    )
