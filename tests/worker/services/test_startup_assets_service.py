...
def service():
    with (
        patch("src.worker.services.startup_assets_service.IntentRouterService"),
        patch("src.worker.services.startup_assets_service.GlossaryService"),
    ):
...
class TestStartupAssetsService:
    def test_ensure_assets_layout(self, service, tmp_path):
        with patch(
            "src.worker.services.startup_assets_service.get_cache_root",
            return_value=tmp_path,
        ):
            with patch(
                "src.worker.services.startup_assets_service.get_subdir_path"
            ) as mock_subdir:
    ...
    async def test_ensure_metadata_indexes_success(self, service, tmp_path):
        with patch(
            "src.worker.services.startup_assets_service.get_cache_file_path"
        ) as mock_path:
