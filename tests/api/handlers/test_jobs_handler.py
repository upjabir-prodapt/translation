...
class TestJobsHandler:
    ...
    async def test_list_jobs(self, handler, mock_service):
        await handler.list_jobs(status="completed", limit=10, offset=0)
        mock_service.list_jobs.assert_called_once_with("completed", 10, 0)

    async def test_cancel_job(self, handler, mock_service):
        req = MagicMock()
        await handler.cancel_job("job1", req)
        mock_service.cancel_job.assert_called_once_with("job1", req)
