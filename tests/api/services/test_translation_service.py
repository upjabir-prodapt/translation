...
class TestTranslationService:
    ...
    async def test_submit_translation_success(self, mock_validate, service, mock_bq, mock_storage, valid_request):
    ...
    async def test_submit_translation_decode_error(self, service, valid_request):
        ...
        # but we can mock it
        with patch("src.api.services.translation_service.base64.b64decode", side_effect=Exception("Decode Error")):
    ...
    async def test_submit_translation_failure_cleanup(self, service, mock_storage, valid_request):
        with patch.object(service, "_normalize_config", side_effect=Exception("Oops")):
            with patch("src.api.services.translation_service.PDFValidator.validate_pdf_bytes", return_value=(b"", {})):
