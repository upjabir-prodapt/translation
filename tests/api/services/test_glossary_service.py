...
class TestGlossaryService:
    ...
    def test_load_local_glossary_json_exists(self, service):
        with patch.object(
            ...
            with patch.object(service, "_local_cache_fresh", return_value=True):
                with patch(
                    "src.api.services.glossary_service.Path.exists", return_value=True
                ):
                    with patch(
                        "src.api.services.glossary_service.Path.read_text",
                        return_value='{"terms": []}',
                    ):
    ...
    def test_download_glossary_json_success(self, service, tmp_path):
        with patch(
            "src.api.services.glossary_service.get_storage_client"
        ) as mock_client:
            mock_blob = MagicMock()
