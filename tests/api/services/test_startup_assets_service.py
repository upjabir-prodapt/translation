...
@pytest.fixture
def service():
    with patch("src.api.services.startup_assets_service.IntentRouterService"), \
...
class TestStartupAssetsService:
    def test_ensure_assets_layout(self, service, tmp_path):
        with patch("src.api.services.startup_assets_service.get_cache_root", return_value=tmp_path):
            with patch("src.api.services.startup_assets_service.get_subdir_path") as mock_subdir:
    ...
    async def test_ensure_metadata_indexes_success(self, service, tmp_path):
        with patch("src.api.services.startup_assets_service.get_cache_file_path") as mock_path:
    ...
    @patch("src.api.services.startup_assets_service.download_async", new_callable=AsyncMock)
    async def test_ensure_metadata_indexes_download(self, mock_down, service, tmp_path):
        with patch("src.api.services.startup_assets_service.get_cache_file_path") as mock_path:
            ...
            with patch.object(Path, "exists", side_effect=[False, False]):
                with pytest.raises(RuntimeError): # It will fail because I didn't mock enough exists() calls or write fast enough
                    await service._ensure_metadata_indexes()

    async def test_run_preflight(self, service):
        with patch.object(service, "ensure_assets_layout", return_value=Path("/tmp")):
            with patch.object(service, "_ensure_metadata_indexes", new_callable=AsyncMock, return_value=["f1", "f2"]):
