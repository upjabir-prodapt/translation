...
from src.api.services.pipeline_orchestrator import PipelineOrchestrator, _PipelineProgressTracker
...
class TestPipelineOrchestrator:
    def test_extract_blob_path(self, orchestrator):
        with patch("src.api.services.pipeline_orchestrator.settings") as mock_settings:
            ...
            assert orchestrator._extract_blob_path("gs://test-bucket/path/to/blob") == "path/to/blob"
            assert orchestrator._extract_blob_path("path/to/blob") == "path/to/blob"

    async def test_update_status(self, orchestrator, mock_bq):
        await orchestrator._update_status("job1", status="completed", error_message="none")
    ...
    @patch("src.api.services.pipeline_orchestrator.extract_attempt_text", return_value=("s", "t"))
    async def test_run_success(self, mock_extract, mock_processor_cls, orchestrator, mock_bq, mock_storage):
        ...
        with patch.object(orchestrator.temp_workspace_service, "create"):
            with patch.object(orchestrator.language_detector, "detect", return_value="en"):
                with patch.object(orchestrator.intent_router, "get_model_chain", return_value=["m1"]):
                    with patch.object(orchestrator.intent_router, "build_intent", return_value="intent1"):
                        with patch.object(orchestrator.glossary_service, "load_domain_glossary", return_value=[]):
