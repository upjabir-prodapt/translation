...
def mock_global_assets():
    with patch("src.api.services.processor_service.get_doclayout_onnx_model_path", return_value=Path("/tmp/fake.onnx")), \
         patch("src.loaders.assets.get_font_and_metadata", return_value=(Path("/tmp/font.ttf"), MagicMock())), \
...
class TestJobProcessorCore:
    ...
    def test_estimate_cost(self, processor):
        cost = processor._estimate_cost(model_id="gemini-1.5-pro", prompt_tokens=1000, completion_tokens=1000)
    ...
    async def test_load_glossary_from_gcs_success(self, mock_mkdir, mock_glossary, mock_repo, processor):
    ...
    @patch("src.api.services.processor_service.extract_attempt_text", return_value=("s", "t"))
...
class TestJobProcessorLanguageDetection:
    ...
    def test_build_translation_config(self, processor):
        ...
        with patch("src.api.services.processor_service.TranslationConfig") as mock_cfg_cls:
            mock_cfg = MagicMock()
            mock_cfg.source_language = "en"
            mock_cfg.target_language = "fr"
            mock_cfg.working_dir = Path("/tmp/work")
            mock_cfg_cls.return_value = mock_cfg
            
            res = processor._build_translation_config(config, Path("/tmp/work"))
            assert res.source_language == "en"
            assert res.target_language == "fr"
            assert str(res.working_dir) == "/tmp/work"
    ...
    async def test_handle_translation_event_progress(self, processor):
        ...
        with patch.object(processor, "_handle_progress_update", new_callable=AsyncMock) as mock_update:
    ...
    async def test_handle_translation_event_finish(self, processor):
        ...
        with patch.object(processor, "_handle_finish_event", new_callable=AsyncMock) as mock_finish:
    ...
    @patch("src.api.services.processor_service.extract_attempt_text", return_value=("source", "target"))
    ...
    async def test_execute_attempt_success(self, mock_write, mock_tokens, mock_build, mock_extract, mock_eval, mock_run, processor):
        mock_trans_config = MagicMock(spec=TranslationConfig)
        mock_trans_config.working_dir = Path("/tmp/work")
        ...
        result, _, _, _, _ = await processor._execute_attempt(
            model_index=0, model_list=["m1"], config={"job_id": "1"},
            output_base_dir=Path("/tmp"), max_attempts=1, judge=MagicMock()
        )
    ...
    async def test_run_single_attempt_success(self, mock_translate, processor):
        ...
        with patch.object(processor, "_handle_translation_event", new_callable=AsyncMock) as mock_handle:
    ...
    @patch("src.api.services.processor_service.Path.mkdir")
    async def test_translate_retry_on_failure(self, mock_mkdir, mock_apply, mock_metadata, mock_exec, processor, mock_config):
        ...
        mock_exec.side_effect = [
            ...
            ({"status": "ok"}, {"attempt_index": 2, "selected_model": "m2"}, MagicMock(), mock_qual, {"token_usage": {}})
        ]
    ...
    @patch.object(JobProcessor, "_build_cover_page_metadata")
    @patch.object(JobProcessor, "_apply_cover_pages")
    async def test_translate_full_flow(self, mock_apply, mock_metadata, mock_exec, processor, mock_config):
        mock_qual = MagicMock()
        mock_qual.final_score = 0.9
        mock_qual.pass_fail = True
        mock_exec.return_value = ({"status": "ok"}, {"attempt_index": 1, "selected_model": "m1"}, MagicMock(), mock_qual, {"token_usage": {}})
    ...
    def test_prepend_cover_page_success(self, mock_open, processor, tmp_path):
        mock_orig = MagicMock(); mock_orig.page_count = 1
        mock_new = MagicMock()
