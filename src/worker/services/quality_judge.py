...
class GoogleADKJudgeAgent:
    """LLM judge agent facade (ADK-style) for omission/hallucination checks."""
    ...
    def __init__(self, model: str | None = None):
        ...
        if Agent is not None:
            # Keep explicit ADK agent metadata in-process for future orchestration.
            self._agent = Agent(
    ...
    def _judge_with_llm(self, source_text: str, translated_text: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "Judge response parse failed; fallback heuristic will be used."
            )
            return {
                "omission_score": _compute_alignment_score(
                    source_text, translated_text
...
def extract_attempt_text(working_dir: Path) -> tuple[str, str]:
    ...
    try:
        ...
        logger.warning("Failed to parse tracking file at %s", tracking_path)
        return "", ""
