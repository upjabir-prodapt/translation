...
import orjson
import pymupdf
import src.worker.doctranslator.format.pdf.high_level
import src.worker.doctranslator.format.pdf.translation_config
...
def get_font_metadata(font_path) -> PdfFont:
    ...
    translation_config = (
        src.worker.doctranslator.format.pdf.translation_config.TranslationConfig(
            *[None for _ in range(4)], doc_layout_model=1
        )
    )
    translation_config.progress_monitor = (
        src.worker.doctranslator.format.pdf.high_level.ProgressMonitor(
            src.worker.doctranslator.format.pdf.high_level.get_translation_stage(
                translation_config
            )
        )
    )
    ...
    buffer = io.BytesIO()
    doc.save(buffer)
    src.worker.doctranslator.format.pdf.high_level.start_parse_il(
        buffer,
        doc_zh=doc,
        resfont="test_font",
