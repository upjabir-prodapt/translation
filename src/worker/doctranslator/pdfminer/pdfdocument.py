...
from src.worker.doctranslator.pdfminer.arcfour import Arcfour
from src.worker.doctranslator.pdfminer.casting import safe_int
from src.worker.doctranslator.pdfminer.data_structures import NumberTree
from src.worker.doctranslator.pdfminer.pdfexceptions import PDFException
from src.worker.doctranslator.pdfminer.pdfexceptions import PDFKeyError
from src.worker.doctranslator.pdfminer.pdfexceptions import PDFObjectNotFound
from src.worker.doctranslator.pdfminer.pdfexceptions import PDFTypeError
from src.worker.doctranslator.pdfminer.pdfparser import PDFParser
from src.worker.doctranslator.pdfminer.pdfparser import PDFStreamParser
from src.worker.doctranslator.pdfminer.pdfparser import PDFSyntaxError
from src.worker.doctranslator.pdfminer.pdftypes import DecipherCallable
from src.worker.doctranslator.pdfminer.pdftypes import PDFStream
from src.worker.doctranslator.pdfminer.pdftypes import decipher_all
...
from src.worker.doctranslator.pdfminer.pdftypes import str_value
from src.worker.doctranslator.pdfminer.pdftypes import stream_value
from src.worker.doctranslator.pdfminer.pdftypes import uint_value
from src.worker.doctranslator.pdfminer.psexceptions import PSEOF
from src.worker.doctranslator.pdfminer.psparser import KWD
from src.worker.doctranslator.pdfminer.psparser import LIT
from src.worker.doctranslator.pdfminer.psparser import literal_name
from src.worker.doctranslator.pdfminer.utils import choplist
from src.worker.doctranslator.pdfminer.utils import decode_text
from src.worker.doctranslator.pdfminer.utils import format_int_alpha
from src.worker.doctranslator.pdfminer.utils import format_int_roman
from src.worker.doctranslator.pdfminer.utils import nunpack
...
class PDFXRef(PDFBaseXRef):
    ...
    def _parse_xref_header_line(
        self, parser: PDFParser, line: bytes
    ) -> tuple[int, int] | None:
        """Parse a subsection header line 'start nobjs'. Returns (start, nobjs) or None if trailer."""
    ...
    def load(self, parser: PDFParser) -> None:
        while True:
            ...
            if line.startswith(b"trailer"):
                parser.seek(pos)
                break
            header = self._parse_xref_header_line(parser, line)
            if header is None:
                parser.seek(pos)
                break
            (start, nobjs) = header
            for objid in range(start, start + nobjs):
                self._parse_xref_entry(parser, objid)
...
class PDFXRefFallback(PDFXRef):
    ...
    def _expand_objstm(self, _parser: PDFParser, objid: int, obj: PDFStream) -> None:
        ...
        try:
            while 1:
                (_, stm_obj) = parser1.nextobject()
                objs.append(cast(int, stm_obj))
        except PSEOF:
            pass
        n = min(n, len(objs) // 2)
    ...
    def load(self, parser: PDFParser) -> None:
        ...
        while 1:
            ...
            if isinstance(obj, PDFStream) and obj.get("Type") is LITERAL_OBJSTM:
                self._expand_objstm(parser, objid, obj)
...
class PDFStandardSecurityHandler:
    ...
    def compute_encryption_key(self, password: bytes) -> bytes:
        ...
        hash_obj.update(self.docid[0])  # 5
        if self.r >= 4:
            if not cast(PDFStandardSecurityHandlerV4, self).encrypt_metadata:
                hash_obj.update(b"\xff\xff\xff\xff")
...
class PDFStandardSecurityHandlerV5(PDFStandardSecurityHandlerV4):
    ...
    def authenticate(self, password: str) -> bytes | None:
        ...
        hash_val = self._password_hash(password_b, self.o_validation_salt, self.u)
        ...
        hash_val = self._password_hash(password_b, self.u_validation_salt)
    ...
    def _normalize_password(self, password: str) -> bytes:
        if self.r == 6:
            ...
            if not password:
                return b""
            from src.worker.doctranslator.pdfminer._saslprep import saslprep
...
class PDFDocument:
    ...
    def __init__(
        ...
        self._load_xrefs(parser, fallback)
        self._process_trailers(password)

    def _load_xrefs(self, parser: PDFParser, fallback: bool) -> None:
        """Load all cross-reference tables from the PDF parser."""
        try:
    ...
    def _process_trailer_encrypt(self, trailer: dict[str, Any], password: str) -> None:
        ...
        # Solves https://github.com/pdfminer/pdfminer.six/issues/594
        self.encryption = (id_value, dict_value(trailer["Encrypt"]))
        self._initialize_password(password)

    def _process_trailers(self, password: str) -> None:
        """Iterate over loaded xrefs, extract encryption/info/root from trailers."""
        for xref in self.xrefs:
            ...
            if "Encrypt" in trailer:
                self._process_trailer_encrypt(trailer, password)
            if "Info" in trailer:
    ...
    _outline_type = tuple[Any, Any, Any, Any, Any]

    def get_outlines(self) -> Iterator[_outline_type]:
        if "Outlines" not in self.catalog:
            raise PDFNoOutlines

        def search(entry: object, level: int) -> Iterator[PDFDocument._outline_type]:
    ...
    def _lookup_name_node(self, d: dict[str, Any], cat: str, key: str | bytes) -> Any:
        ...
        if "Kids" in d:
            for c in list_value(d["Kids"]):
                val = self._lookup_name_node(dict_value(c), cat, key)
                if val:
                    return val
    ...
    def lookup_name(self, cat: str, key: str | bytes) -> Any:
        ...
        return self._lookup_name_node(d0, cat, key)
