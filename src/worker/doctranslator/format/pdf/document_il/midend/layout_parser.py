...
import src.worker.doctranslator.format.pdf.document_il.utils.extract_char
...
class LayoutParser:
    ...
    def generate_fallback_line_layout_for_page(self, page: il_version_1.Page, progress):
        try:
            ...
            char_boxes = src.worker.doctranslator.format.pdf.document_il.utils.extract_char.convert_page_to_char_boxes(
                page
            )
            if not char_boxes:
                return

            clusters = src.worker.doctranslator.format.pdf.document_il.utils.extract_char.process_page_chars_to_lines(
                char_boxes
            )
