...
from src.worker.doctranslator.format.pdf.document_il import PdfCharacter
from src.worker.doctranslator.format.pdf.document_il import PdfCurve
from src.worker.doctranslator.format.pdf.document_il import PdfForm
...
class TypesettingUnit:
    ...
    def __init__(
        ...
        if not (
            ...
            raise ValueError("Only one of chars and formular can be not None")
        self.char = char
        ...
        if unicode:
            if not font_size:
                raise ValueError("Font size must be provided when unicode is provided")
            if not style:
                raise ValueError("Style must be provided when unicode is provided")
            if len(unicode) != 1:
                raise ValueError("Unicode must be a single character")
            if xobj_id is None:
                raise ValueError("Xobj id must be provided when unicode is provided")
    ...
    def render(
        ...
        if self.can_passthrough:
            ...
            if self.scale is None:
                raise RuntimeError("scale must be set, should be set by `relocate`")
            x = self.x
            y = self.y
